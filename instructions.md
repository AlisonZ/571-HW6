...
NOTE: Your representations should employ the Neo-Davidsonian event-style semantics presented in class. In this format, events (primarily, verbs) typically have the form
...
Based on the materials above and the format for implementing semantic attachments in NLTK Ch. 10Links to an external site. , create a set of context-free grammar rules augmented with semantic attachments that are adequate to analyze a small set of English natural language sentences and produce appropriate semantic analyses.
...
Sample grammars may be found in NLTK book Chapter 10 and on Patas under /corpora/nltk/nltk-data/grammars/.

NOTE: The NLTK grammars are provided largely to help with formatting the semantic attachments. Remember that the semantic analysis must employ the Neo-Davidsonian event-style semantics in the class slides. Many of the NLTK grammars do not.
...
Parse the sentence and compositionally create a semantic analysis for it.
Use the nltk.parse.FeatureChartParser class (or similar) to perform this process.
